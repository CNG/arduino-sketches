  ...
  for(uint16_t i=0; i<10; i++) {
    ...
    */
    ...
    for(uint16_t j=0; j<sizeof(times); j++) {
      ...
      uint8_t brightness = map(analogRead(A9), 0, 1023, 0, 255);
      strip.setBrightness(brightness);
      rainbow( 5, wait, space, j%2==0, random(0,8) == 0 && space > 0, random(0,30) == 0 );
    }
  }
...
void setMeterPixelColor(uint16_t n, uint8_t r, uint8_t g, uint8_t b){
  int pixels = strip.numPixels() / 6;
  strip.setPixelColor(pixels-n,r,g,b);
  strip.setPixelColor(pixels+n,r,g,b);
  strip.setPixelColor(pixels*3-n,r,g,b);
  strip.setPixelColor(pixels*3+n,r,g,b);
  strip.setPixelColor(pixels*5-n,r,g,b);
  strip.setPixelColor(pixels*5+n,r,g,b);
}
void setMeterPixelColor(uint16_t n, uint32_t c){
  int pixels = strip.numPixels() / 6;
  strip.setPixelColor(pixels-n,c);
  strip.setPixelColor(pixels+n,c);
  strip.setPixelColor(pixels*3-n,c);
  strip.setPixelColor(pixels*3+n,c);
  strip.setPixelColor(pixels*5-n,c);
  strip.setPixelColor(pixels*5+n,c);
}
...
void rainbow(uint8_t mode, uint8_t wait, uint8_t spaces, boolean dir, boolean solid, boolean storm) {
  ...
  for(color=0; color<256; color++) {
    ...
    for(pixel=0; pixel<strip.numPixels()/6; pixel++) {
      if(mode==1||mode==2){
          ...
          if( pixel % chunkSize == space ){
            if( dir ){
              if( color % chunkSize == space ) {
                ...
                setMeterPixelColor(pixel, strip.Color(0, 0, 0));
              }
            } else {
