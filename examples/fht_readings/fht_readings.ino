...
void setup(){
  ...
  ADC->ADC_RCR=FFT_N/4; // Receive Counter Register
  ADC->ADC_RNPR=(uint32_t)buf[1]; // Receive Next Pointer Register
  ADC->ADC_RNCR=FFT_N/4; // Receive Next Counter Register
  bufn=obufn=1;
...
void loop(){
  ...
  SerialUSB.println(obufn);
  SerialUSB.println(bufn);
  ...
  uint8_t  buckets = 6;
  uint16_t group_n = FFT_N/2 / ( 1 << ( buckets - 1 ) ); // # bins in first bucket
